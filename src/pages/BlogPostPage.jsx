  ...
  if (!post) {
    return (
      <div style={{ minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4rem 1rem' }}>
        ...
        <Link to="/blog" style={{ background: '#6366f1', color: '#fff', padding: '12px 32px', borderRadius: 50, textDecoration: 'none', fontWeight: 600 }}>← Back to Blog</Link>
      </div>
    );
  }
      ...
      <div style={{ paddingTop: '6.5rem' }}>
        <div className="container" style={{ maxWidth: 1200 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 0' }}>
            <Link to="/blog" style={{ color: '#c084fc', fontSize: '0.82rem', textDecoration: 'none', fontWeight: 500 }}>TechGeekz Academy</Link>
            <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: '0.8rem' }}>›</span>
            <Link to="/blog" style={{ color: '#c084fc', fontSize: '0.82rem', textDecoration: 'none', fontWeight: 500 }}>{post.tags[0]}</Link>
          </div>
        </div>
      </div>
        ...
        <div className="container" style={{ maxWidth: 1200 }}>
          <div style={{
            ...
            <div style={{ flex: 1, padding: 'clamp(28px, 4vw, 48px)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(139,92,246,0.12)', borderRadius: 50, padding: '6px 16px', marginBottom: 20, alignSelf: 'flex-start', border: '1px solid rgba(139,92,246,0.25)' }}>
                <span style={{ fontSize: '0.72rem', color: '#c084fc', fontWeight: 600 }}>✦ Expert Reviewed</span>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 42, height: 42, borderRadius: '50%', background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', color: '#fff', fontWeight: 700, border: '2px solid rgba(255,255,255,0.15)' }}>{post.author.charAt(0)}</div>
                  <div>
          ...
          <div className="row">
                  ...
                  <nav style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {tocItems.map((item, i) => (
                      <button key={i} onClick={() => scrollToSection(i)} style={{
                        ...
                        color: activeSection === i ? '#c084fc' : 'rgba(255,255,255,0.5)',
                        fontWeight: activeSection === i ? 600 : 400,
                        borderLeft: activeSection === i ? '3px solid #8b5cf6' : '3px solid rgba(255,255,255,0.06)',
                        background: activeSection === i ? 'rgba(139,92,246,0.08)' : 'transparent',
            ...
            <div className="col-lg-6">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20, flexWrap: 'wrap' }}>
                <span style={{ background: hexToRgba(tagColors[post.tags[0]] || '#6366f1', 0.15), color: tagColors[post.tags[0]] || '#6366f1', padding: '4px 14px', borderRadius: 6, fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase' }}>{post.tags[0]}</span>
              ...
              <div style={{ background: '#12121a', borderRadius: 12, marginBottom: 28, border: '1px solid rgba(255,255,255,0.06)', overflow: 'hidden' }}>
                <button onClick={() => setTocOpen(!tocOpen)} style={{
                  ...
                  <span style={{ color: '#c084fc', fontSize: '1.2rem', transform: tocOpen ? 'rotate(180deg)' : 'rotate(0)', transition: 'transform 0.3s' }}>▾</span>
                </button>
                {tocOpen && (
                  <div style={{ padding: '0 18px 14px' }}>
                    {tocItems.map((item, i) => (
                      <button key={i} onClick={() => { scrollToSection(i); setTocOpen(false); }} style={{
                        all: 'unset', cursor: 'pointer', display: 'block', padding: '6px 0', fontSize: '0.82rem', color: '#c084fc', fontWeight: 500,
                      }}>{item}</button>
                    ))}
                  </div>
                )}
              ...
              <div style={{ marginTop: 36, padding: 24, background: '#12121a', borderRadius: 14, border: '1px solid rgba(255,255,255,0.06)', display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                <div style={{ width: 52, height: 52, borderRadius: '50%', flexShrink: 0, background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', color: '#fff', fontWeight: 700 }}>{post.author.charAt(0)}</div>
                <div>
                  <div style={{ fontSize: '0.95rem', fontWeight: 700, color: '#fff', marginBottom: 2 }}>{post.author}</div>
                  <div style={{ fontSize: '0.78rem', color: '#c084fc', fontWeight: 600, marginBottom: 6 }}>Contributing Writer at TechGeekz</div>
            ...
            <div className="col-lg-3 d-none d-lg-block">
              <div style={{ position: 'sticky', top: 100, paddingLeft: 20 }}>
                ...
                <div style={{ background: '#12121a', borderRadius: 14, padding: 22, border: '1px solid rgba(255,255,255,0.06)', marginTop: 20 }}>
                  ...
                  <button style={{ width: '100%', padding: '9px 0', borderRadius: 8, background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: '#fff', border: 'none', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer' }}>Subscribe</button>
                </div>
                ...
                <div key={rp.id} className="col-lg-4 col-md-6">
                  <Link to={`/blog/post/${slugify(rp.title)}`} style={{ textDecoration: 'none' }}>
                    <div className="glow-card" style={{ background: '#12121a', borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.06)', transition: 'transform 0.3s, box-shadow 0.3s', height: '100%' }}
                      onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-6px)'; e.currentTarget.style.boxShadow = '0 16px 40px rgba(139,92,246,0.15)'; }}
                      ...
                      <div style={{ padding: 20 }}>
                        <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                          {rp.tags.slice(0, 2).map((t, j) => (
                            <span key={j} style={{ fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, color: tagColors[t] || '#6366f1' }}>{t}</span>
                          ))}
                        </div>
