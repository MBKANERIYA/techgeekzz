...
const Navbar = () => {
  ...
  return (
    <header className="position-fixed top-0 start-0 end-0 p-3" style={{ zIndex: 1050 }}>
      <nav
        className={`navbar navbar-expand-lg px-4 py-2 mx-auto ${mobileOpen ? 'nav-mobile-open' : 'rounded-pill'}`}
        style={{
          maxWidth: '1320px',
          background: (scrolled || mobileOpen) ? 'rgba(10, 10, 15, 0.95)' : 'rgba(10, 10, 15, 0.4)',
          backdropFilter: 'blur(20px)',
          WebkitBackdropFilter: 'blur(20px)',
          border: '1px solid rgba(255,255,255,0.08)',
          boxShadow: scrolled ? '0 8px 32px rgba(0,0,0,0.3)' : '0 4px 20px rgba(0,0,0,0.15)',
          transition: 'all 0.4s ease',
          borderRadius: mobileOpen ? '20px' : undefined,
        }}
      >
          ...
          >
            <ul className="navbar-nav mx-auto align-items-lg-center gap-lg-1">
                  ...
                  <div className="mobile-dropdown">
                    {servicesData.map((cat, i) => (
                      <div key={i} className="mb-3">
                        <Link to={cat.titleHref} onClick={closeMobileMenu} className="d-flex align-items-center gap-2 mb-2 text-decoration-none">
                          <span style={{ fontSize: '0.7rem' }}>{cat.icon}</span>
                          <span className="text-white fw-bold" style={{ fontSize: '0.85rem' }}>{cat.title}</span>
                        </Link>
                        <div className="d-flex flex-column gap-1 ps-3">
                          {cat.links.map((link, j) => (
                            <Link key={j} to={link.href} onClick={closeMobileMenu} className="text-decoration-none"
                              style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.8rem', padding: '4px 0' }}>
                              {link.label}
                            </Link>
                          ))}
                        </div>
                    ...
                    <div className="pt-2" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
                      {servicesRight.map((svc, i) => (
                        <Link key={i} to={svc.href} onClick={closeMobileMenu}
              ...
              <li className="nav-item">
                <a className="nav-link fw-medium px-3" href="#" style={navLinkStyle} onClick={closeMobileMenu}>Success Stories</a>
              </li>
              <li className="nav-item">
                <a className="nav-link fw-medium px-3" href="#" style={navLinkStyle} onClick={closeMobileMenu}>Our people</a>
              </li>
              ...
              >
                <a
                  ...
                  style={navLinkStyle}
                  onClick={(e) => {
                    if (isMobile) { e.preventDefault(); toggleDropdown('blog'); }
                  }}
                >
                  Blog
                </a>
